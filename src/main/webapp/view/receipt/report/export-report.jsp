...
<head>
    ...
    <title>Báo cáo xuất kho chi tiết — Warehouse OS</title>
    ...
    <style>
        ...
        table.rpt .col-ref { width: 200px; font-family: var(--font-mono); font-size: 12px; }
...
<div class="app">
    ...
    <div>
        <header class="topbar">
            ...
            <span class="crumb">/ Kho / Phiếu xuất / Báo cáo chi tiết</span>
        ...
        <main>
            <div class="page-head">
                <div class="left">
                    ...
                    <h2 class="page-title">Báo cáo xuất kho chi tiết</h2>
                    <div class="page-sub">Thống kê chi tiết các phiếu xuất kho trong kỳ</div>
                </div>
            </div>
            ...
            <div class="section-head" style="margin-bottom: 16px;">
                <form class="report-filter" method="get" action="${pageContext.request.contextPath}/receipts/report">
                    ...
                    <a class="btn" href="${pageContext.request.contextPath}/receipts/report?action=export&type=EXPORT&fromDate=${fromDate}&toDate=${toDate}<c:if test='${not empty selectedWarehouseId}'>&warehouseId=${selectedWarehouseId}</c:if>">
            ...
            <c:if test="${not empty summary}">
                <div class="rpt-kpis">
                    <div class="rpt-kpi">
                        <div class="label">Tổng số phiếu xuất <span class="dot" style="background:var(--info)"></span></div>
                        ...
                        <c:otherwise>
                            ...
                            <div class="chart-legend" style="display: flex; align-items: center; gap: 20px; padding: 8px 18px 14px; font-size: 12px; color: var(--muted); border-top: 1px solid var(--border); margin-top: 4px;">
                                <span class="legend-item" style="display: inline-flex; align-items: center; gap: 6px;">
                                    <span class="legend-swatch" style="width: 10px; height: 2px; border-radius: 2px; background: var(--accent);"></span>Số phiếu xuất
                                </span>
                                <span class="legend-item" style="display: inline-flex; align-items: center; gap: 6px;">
                                    <span class="legend-swatch" style="width: 10px; height: 2px; border-radius: 2px; background: var(--info);"></span>Số máy xuất
                                </span>
                            </div>
                        </c:otherwise>
                    </c:choose>
            ...
            (function() {
                ...
                var datasets = [
                    { label: 'S\u1ed1 phi\u1ebfu xu\u1ea5t', key: 'receiptCount', color: colors.accent, dashed: false, fill: true },
                    { label: 'S\u1ed1 m\u00e1y xu\u1ea5t',     key: 'machineCount', color: colors.info,   dashed: true,  fill: false }
                ].map(function(ds, idx) {
                ...
                <table class="rpt">
                    <thead>
                        <tr>
                            ...
                            <th class="col-ref">Mã đơn hàng / Thanh lý</th>
                            <th>Ghi chú</th>
                        </tr>
                    </thead>
                    <tbody>
                        <c:choose>
                            <c:when test="${empty rows}">
                                <tr><td colspan="9" class="empty-cell">Không có dữ liệu xuất kho trong khoảng thời gian này.</td></tr>
                            </c:when>
                            <c:otherwise>
                                <c:forEach var="r" items="${rows}" varStatus="vs">
                                    <tr>
                                        ...
                                        <td class="col-code">
                                            <a href="${pageContext.request.contextPath}/export-receipt?action=detail&id=${r.receiptId}">${r.receiptCode}</a>
                                        </td>
...
<td class="col-qty">${r.machineCount}</td>
                                        <td class="col-ref">
                                            <c:choose>
                                                <c:when test="${not empty r.orderCode and not empty r.liquidationCode}">${r.orderCode} / ${r.liquidationCode}</c:when>
                                                <c:when test="${not empty r.orderCode}">${r.orderCode}</c:when>
                                                <c:when test="${not empty r.liquidationCode}">${r.liquidationCode}</c:when>
                ...
                <c:if test="${totalPages > 1}">
                    <c:set var="filterParams" value="type=EXPORT&fromDate=${fromDate}&toDate=${toDate}"/>
