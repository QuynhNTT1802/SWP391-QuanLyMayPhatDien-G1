...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .stat .val .delta { font-family: var(--font-ui); font-size: 11px; font-weight: 600; margin-inline-start: 6px; padding: 1px 5px; border-radius: 3px; vertical-align: middle; }
  .stat .val .delta.up { color: var(--accent); background: var(--accent-soft); }
  .stat .val .delta.down { color: var(--danger); background: var(--danger-soft); }
  ...
  .search-input input {
    width: 100%; border: 1px solid var(--border); background: var(--surface-2); color: var(--fg);
    border-radius: var(--radius-sm); padding: 7px 10px 7px 32px;
    font-size: 13px; font-family: var(--font-ui); font-weight: 500;
  }
  .search-input input::placeholder { color: var(--muted-2); font-weight: 500; }
  .search-input input:focus { outline: none; border-color: var(--accent); background: var(--surface); box-shadow: 0 0 0 3px var(--accent-soft); }
  .search-input svg { position: absolute; inline-size: 14px; block-size: 14px; inset-inline-start: 10px; inset-block-start: 50%; transform: translateY(-50%); stroke: var(--muted); fill: none; stroke-width: 1.8; }
  .filter-select {
  ...
  .bulk-bar {
    ...
    border-bottom: 0; font-size: 13px; font-weight: 600; color: var(--fg);
  }
  body.has-selection .bulk-bar { display: flex; }
  body.has-selection .toolbar { display: none; }
  .bulk-bar .count-pill { background: var(--info); color: var(--bg); padding: 2px 9px; border-radius: 999px; font-family: var(--font-mono); font-size: 12px; font-weight: 700; }
  ...
  .bulk-bar .btn-tiny:hover { background: var(--surface-2); }
  .bulk-bar .btn-tiny.danger { color: var(--danger); border-color: color-mix(in srgb, var(--danger) 30%, transparent); }
  .bulk-bar .btn-tiny.danger:hover { background: var(--danger-soft); }

  .table-card { background: var(--surface); border: 1px solid var(--border); border-top: 0; border-radius: 0 0 var(--radius) var(--radius); overflow: hidden; }
  ...
  table.users th.sortable { cursor: pointer; user-select: none; }
  table.users th.sortable:hover { color: var(--fg); }
  table.users th .sort-ind { display: inline-block; margin-inline-start: 4px; font-family: var(--font-mono); color: var(--muted-2); }
  table.users th.sorted-asc .sort-ind::after { content: '↑'; color: var(--accent); }
  table.users th.sorted-desc .sort-ind::after { content: '↓'; color: var(--accent); }
  table.users tbody tr { cursor: pointer; }
  table.users tbody tr:hover { background: var(--surface-2); }
  table.users tbody tr.selected { background: var(--info-soft); }
  table.users tbody tr.selected:hover { background: color-mix(in srgb, var(--info-soft) 80%, var(--info) 20%); }
  table.users td { vertical-align: middle; }
  td.col-check, th.col-check { width: 36px; padding-inline-end: 0; }
  td.col-actions, th.col-actions { width: 88px; text-align: end; }

  .checkbox {
    appearance: none; width: 15px; height: 15px; border: 1.5px solid var(--border-strong);
    border-radius: 3px; background: var(--surface); cursor: pointer; position: relative;
    transition: all .1s ease; vertical-align: middle;
  }
  .checkbox:hover { border-color: var(--muted); }
  .checkbox:checked { background: var(--accent); border-color: var(--accent); }
  .checkbox:checked::after {
    content: ''; position: absolute; inset-inline-start: 3px; inset-block-start: 0;
    width: 6px; height: 9px; border: solid var(--bg); border-width: 0 1.8px 1.8px 0;
    transform: rotate(45deg);
  }
  .checkbox:indeterminate { background: var(--accent); border-color: var(--accent); }
  .checkbox:indeterminate::after {
    content: ''; position: absolute; inset-inline-start: 2.5px; inset-block-start: 6px;
    width: 8px; height: 1.8px; background: var(--bg);
  }
  ...
  .user-avatar.green { background: oklch(60% 0.16 145); }
  .user-avatar.blue { background: oklch(58% 0.14 250); }
  .user-avatar.orange { background: oklch(64% 0.15 50); }
  .user-avatar.purple { background: oklch(58% 0.16 295); }
  .user-avatar.pink { background: oklch(62% 0.18 0); }
  .user-avatar.teal { background: oklch(58% 0.13 195); }
  .user-avatar.grey { background: oklch(55% 0.01 240); }
  .user-name-block { line-height: 1.3; min-width: 0; }
  .user-name { font-weight: 600; color: var(--fg); }
  .user-email { font-size: 11.5px; color: var(--muted); font-family: var(--font-mono); font-weight: 500; }
  ...
  .pill.role-manager { color: var(--info); border-color: color-mix(in srgb, var(--info) 30%, transparent); background: var(--info-soft); }
  .pill.role-manager .pdot { background: var(--info); }
  .pill.role-keeper { color: var(--accent); border-color: color-mix(in srgb, var(--accent) 30%, transparent); background: var(--accent-soft); }
  .pill.role-keeper .pdot { background: var(--accent); }
  .pill.role-account { color: var(--warn); border-color: color-mix(in srgb, var(--warn) 30%, transparent); background: var(--warn-soft); }
  .pill.role-account .pdot { background: var(--warn); }
  .pill.role-staff { color: var(--fg-soft); border-color: var(--border-strong); background: var(--surface-2); }
  .pill.role-staff .pdot { background: var(--fg-soft); }
  .pill.role-viewer { color: var(--muted); border-color: var(--border); background: var(--surface-2); }
  .pill.role-viewer .pdot { background: var(--muted-2); }
  ...
  .status.active .sdot { background: var(--accent); box-shadow: 0 0 0 3px var(--accent-soft); }
  .status.active { color: var(--fg); }
  .status.pending .sdot { background: var(--warn); }
  .status.pending { color: var(--warn); }
  .status.locked .sdot { background: var(--danger); }
  .status.locked { color: var(--danger); }
  .status.disabled .sdot { background: var(--muted-2); }
  .status.disabled { color: var(--muted); }

  .warehouse-tag { font-size: 11.5px; font-family: var(--font-mono); font-weight: 600; color: var(--fg-soft); background: var(--surface-2); border: 1px solid var(--border); padding: 2px 7px; border-radius: 3px; }

  .last-login { font-size: 12px; color: var(--fg-soft); font-weight: 500; }
  .last-login .when { color: var(--muted); font-size: 11px; font-family: var(--font-mono); }
  ...
  .row-actions .icon-mini:hover { background: var(--surface); border-color: var(--border); color: var(--fg); }
  .row-actions .icon-mini.danger:hover { color: var(--danger); border-color: color-mix(in srgb, var(--danger) 30%, transparent); background: var(--danger-soft); }
  .row-actions svg { width: 14px; height: 14px; stroke: currentColor; fill: none; stroke-width: 1.8; }
  ...
  .pagination .info { color: var(--muted); font-family: var(--font-mono); }
  .pagination .info strong { color: var(--fg); font-weight: 700; }
  .pagination .controls { display: flex; align-items: center; gap: 8px; }
  .pagination .page-btn {
    min-width: 28px; height: 28px; padding: 0 9px; border: 1px solid var(--border);
    background: var(--surface); color: var(--fg-soft); border-radius: var(--radius-sm);
    font-family: var(--font-mono); font-size: 12px; font-weight: 600; cursor: pointer;
  }
  .pagination .page-btn:hover { background: var(--surface-2); color: var(--fg); }
  .pagination .page-btn.active { background: var(--fg); color: var(--bg); border-color: var(--fg); }
  .pagination .page-btn:disabled { opacity: 0.4; cursor: not-allowed; }
  .pagination .page-size {
    border: 1px solid var(--border); background: var(--surface); color: var(--fg);
    border-radius: var(--radius-sm); padding: 4px 22px 4px 8px;
    font-size: 12px; font-weight: 600; font-family: var(--font-ui); cursor: pointer; appearance: none;
  ...
  .empty-state .icon-wrap {
    ...
    background: var(--surface-2); border-radius: 12px; display: grid; place-items: center;
  }
  .empty-state .icon-wrap svg { width: 22px; height: 22px; stroke: var(--muted); fill: none; stroke-width: 1.6; }
  ...
  @media (max-width: 1100px) {
    ...
    .stats-row .stat:nth-child(n+4) { display: none; }
  }
  @media (max-width: 760px) {
    ...
    table.users th:nth-child(6), table.users td:nth-child(6) { display: none; }
  }
</style>
</head>
<body>
