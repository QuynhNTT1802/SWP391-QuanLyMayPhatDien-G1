...
public class SaleOrderDAO extends DBContext implements I_DAO<SaleOrder> {
    ...
    public List<SaleOrder> searchByNameCode(String search, String status, int userId, int loggedUserId) {
        ...
        try {
            ...
            resultSet = statement.executeQuery();
            while (resultSet.next()) {
                SaleOrder order = getFromResultSet(resultSet);
                list.add(order);
            }
        } catch (Exception e) {
    ...
    public java.util.Map<Integer, Integer> getApprovedQuantitiesByGeneratorExcept(int excludeOrderId) {
        ...
        String sql = "SELECT od.generator_id, SUM(od.quantity) AS total_qty "
                ...
                + "WHERE so.status = ? AND so.order_id != ? "
                + "GROUP BY od.generator_id";
        try {
            connection = getConnection();
            statement = connection.prepareStatement(sql);
            statement.setString(1, GlobalUtils.STATUS_APPROVED);
            ...
            resultSet = statement.executeQuery();
            while (resultSet.next()) {
                map.put(resultSet.getInt("generator_id"), resultSet.getInt("total_qty"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public int insert(SaleOrder s) {
        ...
        try {
            ...
            statement.setTimestamp(8, s.getOrderDate() != null ? new java.sql.Timestamp(s.getOrderDate().getTime()) : null);
